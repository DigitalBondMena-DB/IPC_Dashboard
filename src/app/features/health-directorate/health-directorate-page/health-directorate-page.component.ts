...
@Component({
  selector: 'app-health-directorate-page',
  standalone: true,
...
export class HealthDirectoratePageComponent {
  ...
  resource = this._Service.getDirectorates(this.params);
  ...
  onToggle(event: { item: any; field: string; value: boolean }): void {
    this._Service.toggleDirectorate(event.item.id).subscribe({
      next: () => {
        this.resource.reload();
        this._MessageService.add({
          summary: 'Success',
          detail: 'Directorate toggled successfully',
        });
      },
      error: () => {
        this.resource.reload();
        this._MessageService.add({
          severity: 'error',
          summary: 'Error',
          detail: 'Failed to toggle directorate',
        });
      },
    });
  }
  ...
  onEdit(item: any): void {
    this.router.navigate(['/health-directorate/edit', item.id]);
  }

  onCreate(): void {
    this.router.navigate(['/health-directorate/create']);
  }
}
