...
@Component({
  selector: 'app-health-division-page',
  standalone: true,
...
export class HealthDivisionPageComponent {
  ...
  resource = this._Service.getDivisions(this.params);
  ...
  onToggle(event: { item: any; field: string; value: boolean }): void {
    this._Service.toggleDivision(event.item.id).subscribe({
      next: () => {
        this.resource.reload();
        this._MessageService.add({ summary: 'Success', detail: 'Division toggled successfully' });
      },
      error: () => {
        this.resource.reload();
  ...
  onEdit(item: any): void {
    this.router.navigate(['/health-division/edit', item.id]);
  }

  onCreate(): void {
    this.router.navigate(['/health-division/create']);
  }
}
