...
export class AuthoritiesPageComponent {
  ...
  resource = this._Service.getAuthorities(this.params);
  ...
  onToggle(event: { item: any }): void {
    this._Service.toggleAuthority(event.item.id).subscribe({
      next: () => {
        this.resource.reload();
        this._MessageService.add({ summary: 'Success', detail: 'Authority toggled successfully' });
      },
      error: () => {
        this.resource.reload();
        this._MessageService.add({
          severity: 'error',
          summary: 'Error',
          detail: 'Failed to toggle authority',
        });
      },
    });
  }
  ...
  onEdit(item: any): void {
    this.router.navigate(['/authorities/edit', item.id]);
  }
  onCreate(): void {
    this.router.navigate(['/authorities/create']);
  }
}
